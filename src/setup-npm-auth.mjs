...
/**
 * Runs npm whoami for all registríes and throws on authentication failures.
 * @param {Task[]} registries
 * @returns {Promise<void>}
 */
async function verifyRegistryAuth(registries) {
...
/**
 * Main runner for the CLI and action entrypoint.
 * @param {CmdOpts} opts
 * @returns {Promise<Task[]>}
 */
export async function run(opts) {
    ...
    verbose("   🛠️  verifyAuth:", opts.verifyAuth ? "enabled" : "disabled");
    verbose("   🛠️  includes:")
    ...
    if (opts.verifyAuth) {
        await verifyRegistryAuth(registries);
    }
...
`)
        ...
        .option("--verify-auth",
            "Run `npm whoami` for each registry after setting auth to verify credentials. Will throw if any registry fails authentication.",
        )
        .option(
            "-v, --verbose",
